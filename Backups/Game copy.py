...
def skill(root):
    ...
    root.bind('<Escape>', lambda e: goToGame(skillFrame,root))
...
def endFrame(root):
    ...
    endTextBox = endCanvas.create_text(960,300,text=endText,font=("Helvetica", 40),anchor='center')
    diffText = endCanvas.create_text(960,350,text="Choose difficult!",font=("Helvetica", 40),anchor='center')
...
def shop(root):
    ...
    backBtn = Button(shopCanvas,text=" Back ",font=("Helvetica", 15), command= lambda: back(shopFrame,gameFrame)).place(x=1260,y=735)
    
    # Render Frame and Canvas
    shopFrame.pack(fill="both", expand=1)
    shopCanvas.pack(fill="both", expand=1)
    root.bind('<Escape>', lambda e: goToGame(shopFrame,root))
...
def game(root):
    ...
    skillbtn = Button(gameCanvas, text="SCROLL",width=10,border=5,command=lambda:skill(root),font=("Helvetica", 20)).place(relx=0.768, rely=0.8)
    ...
    root.bind('<Escape>', lambda e: close_win(root))
    root.bind('<F2>', lambda x :shop(root))
